# Name: Farris Hossan
# Description: Implementation of Hash Map using a dynamic array, and singly linked list
#              chaining for collision resolution. Chains of key / value are stored in
#              linked list nodes.
...
class HashMap:
    ...
    def resize_table(self, capacity):
        ...
        for i in range(0, self.capacity):
            sll = self._buckets.get(i)
            for node in sll:
                buck = temp.get(self._hash_function(node.key) % capacity)
                if buck.contains(node.key) is None:
                    buck.insert(node.key, node.value)
                    buck.size += 1
                else:
                    buck.contains(node.key).value = node.value
